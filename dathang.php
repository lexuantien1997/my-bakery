...
<div class="row">
	...
	<div class="col-md-8 col-md-off-set-1">
		<h2 class="bg-lienhe" style="padding-left:30px">Đặt hàng</h2>
        <?php
		$tongtt=0;
		$username=$_SESSION['user'];
		$mailnhan='';
		include("PDO/pdo.php");
		$kq=truyvan("select * from user where username='$username'");
		foreach($kq as $m)
			$mailnhan=$m['email'];
		if(isset($_SESSION['giohang']))
		{
			$arr=truyvan('select * from sanpham,loaibanh where sanpham.maloai=loaibanh.maloai');

		?>
		<form method="post">
        <table width="500px" border="1" cellspacing="2" cellpadding="2" align="center">
          <tr align="center" style="background-color:#900; color:#FFF; font-weight:bold; font-size:20px; height:30px;">
            <td width="41">STT</td>
            <td width="183"> Sản phẩm</td>
            <td width="59">Số lượng</td>
            <td width="67">Đơn giá</td>
          </tr>
          <?php
		  $gia=array();
		  $dem=0;
		  $noidungmail='<h1>Cảm ơn quý khách đã mua hàng!</h1><table width="500px" border="1" cellspacing="2" cellpadding="2" align="center" style="border-collapse:collapse" >
          <tr align="center" style="background-color:#900; color:#FFF;">
            <td>STT</td>
            <td> Sản phẩm</td>
            <td>Số lượng</td>
            <td>Đơn giá</td>
		  ...
		  ?>
           <tr align="center" style="height:30px">
            <td><?php $noidungmail.="<tr><td>$dem</td>"; echo $dem?></td>
            <td><?php $noidungmail.="<td>{$node['tensp']}</td>"; echo $node['tensp']?></td>
            
          
          
            <td><?php $noidungmail.="<td>$v</td>"; echo $v?></td>
            <td><?php $noidungmail.="<td>{$node['gia']}</td></tr>"; echo number_format((int)$node['gia'])?> VNĐ</td>
          </tr>
          <?php
					}
				}
			}
			$noidungmail.="<td colspan='6'>Tổng thành tiền:$tongtt VNĐ</td>
</tr></table>";
		  ?>
          <tr align="right" style="height:35px">
                <td colspan="6">Tổng thành tiền: <?php echo number_format($tongtt)?> VNĐ </td>
              </tr>
          <tr align="center">
            <td colspan="6">
           <input type="submit" name="btnxacnhan"  value="Xác nhận" style="font-family:Dancing Script; font-size: 18px; font-weight:bold; border-radius: 10px; width: 80px; height: 30px; background-color: #FF0004; color: #fff" /> &nbsp;
           <input type="button" name="btnmuatiep"  value="Mua tiếp" style="font-family:Dancing Script; font-size: 18px; font-weight:bold; border-radius: 10px; width: 80px; height: 30px; background-color: #FF0004; color: #fff" onClick="window.location='sanpham.php'"/>
		...
		if(isset($_POST['btnxacnhan']))
		  {
			  ...
			  $insertGD=insert_update("insert into giaodich values(null,$iduser,CURRENT_DATE,$tongtt)");
			  //insert chi tiết hoá đơn
			  if($insertGD)
			  {
				  $magiaodich=get_id_giaodich();
				  $stt=0;
				  foreach($_SESSION['giohang'] as $k=>$v)
				  {
					  $CTGD=insert_update("insert into chitietgiaodich values ($magiaodich,$k,$v,$gia[$stt]*$v)");
					 
					  $stt++;
				  }
			  }
			  if($insertGD==true && $CTGD==true)
			  {
				  unset($_SESSION['giohang']);
				  require_once('guimail.php');
				  echo("<script>alert('Đặt hàng thành công, kiểm tra lại mail')</script>");
