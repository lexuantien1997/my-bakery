...
<div id="jssor_1" style="position:relative;margin:0 auto;top:-50px;left:0px;width:980px;height:245px;overflow:hidden;visibility:hidden;"> 
    ...
    <div class="col-md-8 col-md-off-set-1">
		<h2 class="bg-lienhe" style="padding-left:30px">Đăng ký</h2>
        <?php
        include("PDO/pdo.php");
		$hoten=$user=$pass=$email=$sdt=$diachi='';
		if(isset($_POST['btn_gui']))
		{
			...
			{
				$flag=0;
				echo "<script>alert('Chưa nhập tên tài khoản')</script>";
			}
			if($pass=='')
			{
				$flag=0;
			...
			{
				$flag=0;
				echo "<script>alert('Chưa nhập họ tên')</script>";
			}
			if($repass!=$pass)
			{
				$flag=0;
			...
			if($flag==1)
			{
				$kq=insert_update("insert into user values(null,'$hoten','$user','$pass','$email','$sdt','$diachi','$gender','$birday');");
				if($kq==false)
				{
				...
				else
				{
					...
					echo '<meta http-equiv="refresh" content="0,dangnhap.php">';
				}
			}
		}
		
		?>
		<form method="post" action="dangky.php" style="color:#ff0026;" >
        <div class="col-md-3" style="color:#000; padding-left:30px">Tài khoản(*)</div>
        <div class="col-md-9">
        <input type="text" name="user" size="58" placeholder=" Nhập tài khoản" class="vienhong type" value="<?php echo $user?>" />
        </div>
        <div class="col-md-3" style="color:#000; padding-left:30px">Mật khẩu(*)</div>
        <div class="col-md-9">
        <input type="password" name="pass" size="58" placeholder=" Nhập mật khẩu" class="vienhong type" value=""/>
        </div>
        <div class="col-md-3" style="color:#000; padding-left:30px">Nhập lại mật khẩu(*)</div>
        <div class="col-md-9">
        <input type="password" name="repass" size="58" placeholder=" Nhập lại mật khẩu" class="vienhong type" value=""/>
        </div>
    	<div class="col-md-3" style="color:#000; padding-left:30px">Họ &amp; tên(*)</div>
        <div class="col-md-9">
        <input type="text" name="hoten" size="58" placeholder=" Nhập họ và tên chính xác" class="vienhong type" value="<?php echo $hoten?>"/>
        </div>
        <div class="col-md-3" style="color:#000; padding-left:30px">Địa chỉ</div>
        <div class="col-md-9">
        <input type="text" name="diachi" size="58" placeholder=" Nhập địa chỉ" class="vienhong type" value="<?php echo $diachi?>"/>
        </div>
        <div class="col-md-3" style="color:#000; padding-left:30px">Số điện thoại</div>
        <div class="col-md-9">
        <input type="tel" name="sdt" size="58" maxlength="12" placeholder=" Nhập số điện thoại" class="vienhong type" value="<?php echo $sdt?>"/>
        </div>
        <div class="col-md-3" style="color:#000; padding-left:30px">Giới tính</div>
        <div class="col-md-9">
            <input type="radio" name="gender" value="Name"> Nam<br>
            <input type="radio" name="gender" value "Nữ" checked="checked" size="58" > Nữ<br>
        </div> 

        <div class="col-md-3" style="color:#000; padding-left:30px">Ngày sinh</div>
        <div class="col-md-9">
            <input type="date" name="birdday" size="58"  value="1997-10-25" />  
        </div>
        <div class="col-md-3" style="color:#000; padding-left:30px">Email</div>
        <div class="col-md-9">
        <tr>
    		<td colspan="2"><input type="email" name="email" placeholder=" Nhập email" size="58" value="<?php echo $email?>" ></td>
  		</tr>
